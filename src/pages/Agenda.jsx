...
import {
  ...
  CheckCircle2, XCircle, MessageCircle, Trash2, BadgeCheck, Loader2
} from 'lucide-react'
...
export default function Agenda() {
  ...
  const cargar = async () => {
    ...
    try {
      ...
      if (montado.current) setCitas(data || [])
    } catch {
    ...
    <div className="space-y-5">
      ...
      <div className="card p-2 flex items-center justify-between">
        <button onClick={() => navegar(-1)}
          className="grid place-items-center w-11 h-11 rounded-xl2 hover:bg-clinic-50 text-clinic-500">
          <ChevronLeft size={20} />
        </button>
            ...
            return (
              <div key={f} className="card overflow-hidden">
                <button onClick={() => abrirNueva(f)}
                  ...
                  <span className="font-display font-bold text-clinic-700 capitalize">
                    {format(d, "EEEE dd", { locale: es })}
                  </span>
                  <span className="text-[12px] text-clinic-400">
                    {lista.length} cita(s) · <span className="text-clinic-500 font-semibold">+ agregar</span>
                  </span>
                </button>
                {lista.length > 0 && (
                  <div className="divide-y divide-clinic-50">
                    {lista.map(c =>
                      <FilaCita key={c.id} c={c} onEstado={cambiarEstado} onEliminar={eliminar} compacta />
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input type="time" className="field" value={form.hora}
...
function VistaDia({ citas, onEstado, onEliminar, onNueva }) {
  if (citas.length === 0)
    return (
      <Vacio icon={CalendarDays} titulo="Día libre"
    ...
    <div className="space-y-2.5">
      {citas.map(c => (
        <div key={c.id} className="card animate-fade-up">
          <FilaCita c={c} onEstado={onEstado} onEliminar={onEliminar} />
        </div>
      ))}
    </div>
  )
...
function FilaCita({ c, onEstado, onEliminar, compacta }) {
  ...
  const wsp = c.pacientes?.celular
    ? linkWhatsAppPaciente(
        c.pacientes.celular,
        `Hola ${c.pacientes?.nombres}, te recordamos tu cita en Movimiento Koray el ${c.fecha} a las ${hora12(c.hora)}.`
      )
    : null
  return (
    <div className={compacta ? 'px-4 py-3' : 'p-3.5'}>
      <div className="flex items-center gap-3">
        <div className="grid place-items-center w-14 shrink-0">
          ...
          <span className="font-display font-bold text-clinic-700 text-[13px] mt-0.5">
            {c.hora?.slice(0,5)}
          </span>
        </div>
      ...
      {abierto && (
        <div className="mt-3 pt-3 border-t border-clinic-50 flex flex-wrap gap-2 animate-fade-up">
          <button onClick={() => onEstado(c.id, 'Confirmada')} className="btn-ghost flex-1 min-w-[120px]">
            <CheckCircle2 size={16} /> Confirmar
          </button>
          <button onClick={() => onEstado(c.id, 'Asistida')} className="btn-mint flex-1 min-w-[120px]">
            <BadgeCheck size={16} /> Asistió
          </button>
          <button onClick={() => onEstado(c.id, 'Cancelada')} className="btn-soft-danger flex-1 min-w-[120px]">
            <XCircle size={16} /> Cancelar
          </button>
          {wsp && (
            <a href={wsp} target="_blank" rel="noreferrer" className="btn-ghost flex-1 min-w-[120px]">
              <MessageCircle size={16} /> WhatsApp
            </a>
          )}
          <button onClick={() => onEliminar(c.id)} className="btn-soft-danger">
            <Trash2 size={16} />
          </button>
        </div>
      )}
