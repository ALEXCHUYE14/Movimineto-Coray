...
import { Tags, MessageCircle, PackageCheck, Activity, AlertCircle } from 'lucide-react'
    ...
    const cargar = async () => {
      try {
        const { data, error: err } = await supabase
          .from('servicios_precios').select('*').eq('activo', true)
    ...
    <div className="space-y-5">
      <a href={wsp} target="_blank" rel="noreferrer"
        ...
        <div className="flex-1">
          <p className="font-display font-bold">Agendamiento rápido</p>
          <p className="text-white/80 text-[13px]">Escribir al consultorio · 996 113 188</p>
        </div>
      </a>

      {cargando ? (
        <div className="card p-8 text-center text-clinic-300">Cargando catálogo...</div>
      ) : error ? (
        <div className="card p-8 text-center flex flex-col items-center gap-3">
          ...
          <div>
            <p className="font-display font-bold text-clinic-700">No se pudo cargar el catálogo</p>
            <p className="text-sm text-clinic-400 mt-1">Verifica tu conexión e intenta recargar la página.</p>
          </div>
        </div>
      ) : servicios.length === 0 ? (
        <Vacio icon={Tags} titulo="Catálogo vacío" descripcion="No hay servicios registrados." />
      ) : categorias.map(cat => (
        <section key={cat}>
          ...
          <div className="space-y-2.5">
            {servicios.filter(s => s.categoria === cat).map(s => (
              <div key={s.id} className="card p-4 flex items-center gap-3 animate-fade-up">
                <div className={`grid place-items-center w-11 h-11 rounded-xl2 shrink-0 ${
                  s.es_paquete ? 'bg-mint-50 text-mint-600' : 'bg-clinic-50 text-clinic-500'}`}>
                  {s.es_paquete ? <PackageCheck size={20} /> : <Activity size={20} />}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="font-semibold text-clinic-800">{s.nombre_servicio}</p>
                  {s.es_paquete && s.sesiones && (
                    <p className="text-[12px] text-mint-600 font-semibold">{s.sesiones} sesiones</p>
                  )}
                </div>
